...
def save_top_view_png(path: str,
    ...
    颜色约定：
      - 蓝色：有效点（包含内部固定点 + 可移动且成功进入可行位置的点）
      - 黑色：无效点（当前在外部，且通过允许的离散移动仍无法进入并满足吸附条件）
      - 红色：最终 ring 点
    """
    os.makedirs(os.path.dirname(path), exist_ok=True)
    ...
    if valid3d is not None and len(valid3d) > 0:
        pl.add_mesh(pv.PolyData(valid3d), color="deepskyblue", point_size=11, render_points_as_spheres=True)
    ...
    text_lines = [
        "Top View | Blue:valid | Black:invalid | Red:ring | Green:offset70"
    ]
    if title:
    ...
    for dx in d_vals:
        for dy in d_vals:
            ...
            if save_views and view_dir:
                ...
                valid_flag = keep_flag
                invalid_flag = ~keep_flag
                valid3d = lift_points_to_surface(nodes_xyz, moved2d[valid_flag], k=12) if np.any(valid_flag) else np.empty((0, 3))
                invalid3d = lift_points_to_surface(nodes_xyz, grid2d_all[invalid_flag], k=12) if np.any(invalid_flag) else np.empty((0, 3))
...
def visualize_scene(nodes_xyz: np.ndarray,
    ...
    pl.add_text(
        "LightGray:grid | Orange:inner-fixed | Cyan:near-edge movable valid | Black:invalid | Magenta:ring(moved) | Red:ring(base) | Yellow:nodes covered\n"
...
def main():
    ...
    print(f"✅ Valid supports: {int(valid_flag.sum())} / {len(grid2d_all)}  (含内部固定 + 可移动成功)")
    print(f"⚫ Invalid supports (black): {int(invalid_flag.sum())} / {len(grid2d_all)}  (外部且通过移动仍无法进入)")
    ...
    valid3d = lift_points_to_surface(nodes, moved2d[valid_flag], k=12) if valid_flag.any() else np.empty((0, 3))
    ...
    save_top_view_png(
        ...
        ring3d=ring_moved3d,
        title=f"BEST | dx={best_dx*1000:.0f}mm dy={best_dy*1000:.0f}mm | valid={int(keep_flag.sum())} invalid={int((~keep_flag).sum())} ring={len(ring_moved2d)} movable={int(movable_candidate_flag.sum())} inner={int(inner_fixed_flag.sum())}"
    )
