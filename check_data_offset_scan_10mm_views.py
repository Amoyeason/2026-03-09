...
def save_top_view_png(path: str,
                      ...
                      inside3d: np.ndarray | None = None,
                      ok3d: np.ndarray | None = None,
                      ring3d: np.ndarray | None = None,
                      title: str = ""):
    """
    保存俯视图 PNG，便于对 625 个扫描结果做人工对比。
    """
    os.makedirs(os.path.dirname(path), exist_ok=True)
    ...
    if inside3d is not None and len(inside3d) > 0:
        pl.add_mesh(pv.PolyData(inside3d), color="deepskyblue", point_size=7, render_points_as_spheres=True)

    if ok3d is not None and len(ok3d) > 0:
        pl.add_mesh(pv.PolyData(ok3d), color="purple", point_size=10, render_points_as_spheres=True)
    ...
    text_lines = [
        "Top View | Gray:all grid | Blue:inside | Purple:ok | Red:ring | Green:offset70"
    ]
    if title:
...
def scan_grid_offsets(min_xy, max_xy, spacing,
    ...
    for dx in d_vals:
        for dy in d_vals:
            ...
            # 为了“快速扫描”，默认只在 ok 点集合上运行 move/ring（减少计算量）
            base_for_move = grid2d_all[ok_flag]
            moved2d_kept = np.empty((0, 2), dtype=float)
            chosen_idx = np.empty((0,), dtype=int)

            if len(base_for_move) == 0:
                keep_flag = np.zeros((0,), dtype=bool)
                ring_n = 0
            else:
                clearance_need = FIXTURE_RADIUS_M + SAFETY_MARGIN_M
                moved2d, keep_flag, infos = move_points_towards_offset_with_priority(
                    base_points2d=base_for_move,
            ...
            if save_views and view_dir:
                ...
                ok3d = lift_points_to_surface(nodes_xyz, grid2d_all[ok_flag], k=12) if ok_flag.any() else np.empty((0, 3))
                ...
                if SCAN_VIEW_FILENAME_WITH_SCORE:
                    filename = f"view_dx{dx:.3f}_dy{dy:.3f}_ok{ok_n:03d}_kept{kept_n:03d}_ring{ring_n:03d}.png"
                else:
                    filename = f"view_dx{dx:.3f}_dy{dy:.3f}.png"

                title = f"dx={dx*1000:.0f}mm  dy={dy*1000:.0f}mm | ok={ok_n} kept={kept_n} ring={ring_n} score={score}"
                save_top_view_png(
...
def export_offset_scan_csv(path: str, rows: list[dict]):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    cols = ["dx_m", "dy_m", "x0_m", "y0_m", "ok_n", "kept_n", "ring_n", "score"]
...
def main():
    ...
    # 保存“最优偏移”的最终俯视图
    inside3d = lift_points_to_surface(nodes, grid2d_all[inside_flag], k=12) if inside_flag.any() else np.empty((0, 3))
    ok3d = lift_points_to_surface(nodes, grid2d_all[ok_flag], k=12) if ok_flag.any() else np.empty((0, 3))
    save_top_view_png(
        ...
        ok3d=ok3d,
        ring3d=ring_moved3d,
        title=f"BEST | dx={best_dx*1000:.0f}mm dy={best_dy*1000:.0f}mm | ok={int(ok_flag.sum())} kept={int(keep_flag.sum())} ring={len(ring_moved2d)}"
    )
