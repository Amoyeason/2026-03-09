...
# 动态边缘带：在当前孔阵偏移下，薄板边缘带内的 POGO 允许 XY 移动；内部 POGO 只允许升降
EDGE_BAND_M = 0.12
...
# 优化目标： "ok"=最大化可吸附孔位数(ok_flag)；"kept"=最大化贴近偏移线后保留数(keep_flag)；"ring"=最大化最终一圈支撑数量
OFFSET_SCAN_OBJECTIVE = "ok"
# 目标相同的情况下的次级排序（从左到右）
...
def split_grid_states(inside_flag: np.ndarray, clearance: np.ndarray, edge_band_m: float = EDGE_BAND_M):
    edge_movable_flag = inside_flag & (clearance <= edge_band_m)
    inner_fixed_flag = inside_flag & (clearance > edge_band_m)
    outside_flag = ~inside_flag
    return outside_flag, edge_movable_flag, inner_fixed_flag
...
def process_support_points_dynamic(grid2d_all: np.ndarray,
    ...
    outside_flag, edge_movable_flag, inner_fixed_flag = split_grid_states(inside_flag, clearance, edge_band_m=edge_band_m)
    ...
    # 1) 边缘带：允许 XY 离散移动
    edge_idx = np.where(edge_movable_flag)[0]
    if len(edge_idx) > 0:
        moved_edge2d, keep_edge, infos_edge = move_points_towards_offset_with_priority(
            base_points2d=grid2d_all[edge_idx],
        ...
        moved2d[edge_idx] = moved_edge2d
        keep_flag[edge_idx] = keep_edge
        for gi, row in zip(edge_idx, infos_edge):
            row = dict(row)
            row["state"] = "edge_movable"
            infos[gi] = row

    # 2) 内部：只允许升降，不允许 XY 移动
    ...
    if len(inner_idx) > 0:
        ...
        for local_i, gi in enumerate(inner_idx):
            ...
            feasible0 = clear0 >= need
            keep = feasible0 and (dist0 <= MAX_DIST_TO_OFFSET_M)
            keep_flag[gi] = keep
            infos[gi] = {
                "index": int(gi),
                ...
                "keep": int(keep),
                "reason": "inner_fixed_keep" if keep else ("inner_fixed_no_clearance" if not feasible0 else "inner_fixed_too_far_from_offset"),
                "state": "inner_fixed"
            }

    # 3) 板外：不参与
    outside_idx = np.where(outside_flag)[0]
    if len(outside_idx) > 0:
        d0, _ = offset_tree.query(grid2d_all[outside_idx], k=1)
        for local_i, gi in enumerate(outside_idx):
            p = grid2d_all[gi]
            infos[gi] = {
                "index": int(gi),
                "x0": float(p[0]), "y0": float(p[1]),
    ...
    return {
        ...
        "edge_movable_flag": edge_movable_flag,
...
def save_top_view_png(path: str,
    ...
    颜色约定：
      ...
      - 青蓝色：可移动且可到达内部/偏移线附近的点（edge_movable & keep）
    ...
    text_lines = [
        "Top View | Orange:inner-fixed | Cyan:movable-valid | Black:invalid | Red:ring | Green:offset70"
    ]
    if title:
...
def scan_grid_offsets(min_xy, max_xy, spacing,
    ...
    for dx in d_vals:
        for dy in d_vals:
            ...
            infos = dyn["infos"]
            edge_movable_flag = dyn["edge_movable_flag"]
            ...
            edge_n = int(edge_movable_flag.sum())
            ...
            if save_views and view_dir:
                ...
                movable_valid3d = lift_points_to_surface(nodes_xyz, moved2d[edge_movable_flag & keep_flag], k=12) if np.any(edge_movable_flag & keep_flag) else np.empty((0, 3))
                invalid3d = lift_points_to_surface(nodes_xyz, grid2d_all[edge_movable_flag & (~keep_flag)], k=12) if np.any(edge_movable_flag & (~keep_flag)) else np.empty((0, 3))
...
def export_support_moves_csv(path: str, infos: list[dict]):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    cols = ["index", "state", "x0", "y0", "x1", "y1", "dir", "move_mm",
...
def visualize_scene(nodes_xyz: np.ndarray,
                    ...
                    offset3d: np.ndarray,
                    grid3d_all: np.ndarray,
                    inner_fixed3d: np.ndarray,
                    movable_valid3d: np.ndarray,
    ...
    if inner_fixed3d is not None and len(inner_fixed3d) > 0:
        pl.add_mesh(pv.PolyData(inner_fixed3d), color="orange", point_size=11, render_points_as_spheres=True)
    ...
    pl.add_text(
        "LightGray:grid | Orange:inner-fixed | Cyan:movable-valid | Black:invalid | Magenta:ring(moved) | Red:ring(base) | Yellow:nodes covered\n"
...
def main():
    ...
    invalid_flag = edge_movable_flag & (~keep_flag)
    print(f"🧩 Fixed hardware grid: rows={GRID_ROWS} cols={GRID_COLS} total={GRID_ROWS*GRID_COLS}")
    print(f"🟦 Inside covered: {int(inside_flag.sum())} | edge-movable(dynamic): {int(edge_movable_flag.sum())} | inner-fixed(dynamic): {int(inner_fixed_flag.sum())} | outside: {int(outside_flag.sum())}")
    ...
    movable_valid_flag = edge_movable_flag & keep_flag
    invalid_flag = edge_movable_flag & (~keep_flag)

    inner_fixed3d = lift_points_to_surface(nodes, grid2d_all[inner_fixed_flag], k=12) if inner_fixed_flag.any() else np.empty((0, 3))
    movable_valid3d = lift_points_to_surface(nodes, grid2d_all[movable_valid_flag], k=12) if movable_valid_flag.any() else np.empty((0, 3))
    ...
    save_top_view_png(
        ...
        ring3d=ring_moved3d,
        title=f"BEST | dx={best_dx*1000:.0f}mm dy={best_dy*1000:.0f}mm | orig_ok={int(ok_flag.sum())} kept={int(keep_flag.sum())} ring={len(ring_moved2d)} edge={int(edge_movable_flag.sum())} inner={int(inner_fixed_flag.sum())}"
    )
